...
import { useUserInfo, useTitle } from '../../../store/selectors';
...
function Header() {
  ...
  const deleteUserInfo = useAppStore((s) => s.deleteUserInfo);
  const { confirm } = Modal;

  // 生命周期
  useEffect(() => {
    // 获取天气预报(高德接口)
    reqweather().then((res: any) => {
      setlives(res.lives[0]);
    });
  ...
  function logout() {
    confirm({
      title: (
        <h1 style={{ fontSize: '20px' }}>确定退出？</h1>
      ),
      content: (
  ...
  function getPathTitle(
    path: string,
    list: Array<{ key: string; title: string; children?: any[] }>
  ) {
    list.forEach((item) => {
      if (item.children) {
  ...
  return (
    <header className="header">
      <div className="header_top">
        ...
        <Button type="link" className="logout" onClick={logout}>
          退出登录
        </Button>
      </div>
